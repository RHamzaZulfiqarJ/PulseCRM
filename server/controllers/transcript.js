...
export const createtranscript = async (req, res, next) => {
    try {
        ...
        const { employeeName, designation, phone, salaryMonth, salaryType, totalSalary, lateArrivals, halfDays, dayOffs, netSalary, amountPerDayOff } = req.body
        
        const newTranscript = new Transript({  employeeName, designation, phone, salaryMonth, salaryType, totalSalary, lateArrivals, halfDays, dayOffs, netSalary, amountPerDayOff  })
...
export const updateTranscript = async (req, res, next) => {
    try {
        ...
        const { TransriptId } = req.params
        const { employeeName, designation, phone, salaryMonth, totalSalary, salaryType, lateArrivals, halfDays, dayOffs, netSalary, amountPerDayOff } = req.body
        
        const updatedTranscript = await Transript.findByIdAndUpdate(TransriptId, { employeeName, designation, phone, salaryMonth, salaryType, totalSalary, amountPerDayOff, lateArrivals, halfDays, dayOffs, netSalary }, { new: true })
